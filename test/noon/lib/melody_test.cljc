...
(deftest main
  ...
  (testing "gen"
    ...
    (score (m/simple-line 32 (one-of s1 s1- d1 d1-))
                   (adjust 4))

    (score (m/simple-line 32 (one-of s1 s1- (tup d1 d0 s2-) (tup d1- d0 s2)))
                   (adjust 8))
    ...
    (score {:description "another way to build a melodic line from a bunch of randomly chosen updates"}
