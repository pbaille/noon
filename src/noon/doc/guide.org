...
- writing midi file
- setting the tempo
- setting various midi setup parameters
- writing a music xml and/or pdf file (musescore needed)

For a complete description of available options, please see the dedicated section *TODO*

From now on, as a shorthand for examples, we will use the =play= function:
...
#+begin_src clojure :proll
(play (patch :clarinet) (cat C0 E0 G#0 B0))
#+end_src

#+begin_src clojure :proll
(play (patch :vibraphone) [dur:4 (cat C0 E0 G0 #{D1 B0})])
#+end_src
...
(play
 (chans
  [(patch :ocarina) dur:2 (cat G0 Eb0 C0 G-1 F0 D0 A-1 F-1)]
  [(patch :vibraphone) dur2 vel3 (cat #{C0 Eb0 G0} #{A-1 F0 D0})]
  [(patch :acoustic-bass) (cat [dur3 C-2] G-2)])
 (dup 4))
#+end_src
...
It is now time to brings intervals into the equation, pitches were nice for introduction purposes but lacks the flexibility that intervals have. When a musician thinks about music, he do not think in precise pitches most of the time, he more often thinks of scales, intervals, degrees, melodic contour etc... Those higher level abstractions are available in this system and in fact it is the whole point of it. Some really nice libraries already exists to deal with low levels aspects of music notation and sound synthesis.
...
#+begin_src clojure :pp
{"going up 3 semitones from wherever we are." (c-step 3),
 "going down one semitone." (c-step -1)}
#+end_src
...
#+begin_src clojure :proll
(play dur:4 (root :Eb) (scale :hungarian) (cat d0 d1 d2 d3 d4 d5 d6 d7))
#+end_src
...
paraphrasing wiki: In music, an octave is the interval between one musical pitch and another with double its frequency. The octave relationship is a natural phenomenon that has been referred to as 'the basic miracle of music'. The interval between the first and second harmonics of the harmonic series is an octave.
...
#+begin_src clojure :proll
(play dur:8 (cat c0 c3 c6) (cat c0 c2 c3 c5))
#+end_src
...
=fit= is used to make a transformation fit the current duration of the score. The 2 previous transformations introduced: =dup= and =rep=, are actually changing the score length. Sometimes we want to transform our score in place, stretching or compressing it, in the same way =tup= is acting.
...
#+begin_src clojure :pp
(= (mk (tup c0 c3 c8)) (mk (fit (cat c0 c3 c8))))
#+end_src
...
(play
 ...
 (par (tup d0 d2 d4 o1) [vel3 (par> o1 d4) (fit (rep 8 d1))] o1-))
#+end_src

Parallels transformations can be used anywhere of course. Here inside a =tup=.

#+begin_src clojure :proll
(play o1 (tup c0 #{c15 c10} c9 #{c6 c4}) (rep 3 c3))
#+end_src

#+begin_src clojure :proll
(play (par (rep 12 c1) (rep 12 c1-)))
#+end_src
...
#+begin_src clojure :proll
(play (patch :string-ensemble-1) o2- (par> c0 c7 c7 c7 c7 c7)) ; piling perfect fifths.
#+end_src
...
increment the current midi channel.
...
(play
 (patch :flute)
 (tracks (tup> c0 c5 c5 c5- c2- c7-) (tup> c0 c2- c5 c5))
 (dup 4))
#+end_src
...
All the transformation we've seen so far are acting on a score to produce another score, but sometimes what we need is to apply a transformation on each event of a score, for this we are using the =$= function.

As an illustration, here those two fragments:

#+begin_src clojure :proll
(play (cat c0 c1 c2 c3) (tup c0 o1)) ; each member of this `tup` form receives and operate on the whole score
#+end_src

#+begin_src clojure :proll
(play (cat c0 c1 c2 c3) ($ (tup c0 o1))) ; each event of the score is transformed using this `tup` transformation.
#+end_src
...
(play
 ...
 (rep
  60
  (any-that (within-pitch-bounds? :C-1 :C1) c2 c5 c7 c2- c5- c7-))) ; a melody of 60 notes using the 6 given intervals but remaining in the given pitch bounds. 
#+end_src
...
#+begin_src clojure :proll
(play (cat (origin :C0) (origin :E0) (origin :G#0)) ($ (rup 6 s1)))
#+end_src
...
#+begin_src clojure :proll
(play (patch :trumpet) (cat I IV V I) ($ (tup s0 s1 s2)))
#+end_src
...
#+begin_src clojure :proll
(play (patch :woodblock) dur:4 (cat _ (tup _ _) _ dur2 _ _))
#+end_src

We can also use the =dupt= function if we prefer.

#+begin_src clojure :proll
(play (patch :woodblock) dur:4 (cat _ (dupt 2) _ dur2 _ _))
#+end_src
...
(play
 (chans
  ...
  [(patch :woodblock)
   (r/durtup 2 1 1 4)
   (cat _ (r/rotation 1/2) (r/rotation 1/4) (r/rotation -1/4))]))
#+end_src
  ...
  [(patch :ocarina)
   ...
   ($
    [(shuftup s0 s1 s2)
     ($
      (one-of
       (tup s0 (shufcat (one-of c1- s-) s+) s0)
...
This transformation helps you to zip a melody on the current chord progression. This way you don't have to worry at all about the chords, just write a melody it will be adjusted to chord changes.
...
#+begin_src clojure :proll
(play
 [(scale :harmonic-minor) (tup I IV VII I) (h/align-contexts)]
 (h/grid-zipped
  [(patch :ocarina) (tup s0 s1 [s2 (cat d1 d1- _)] s1) (dupt 4)])
 (dup 2)
 (adjust {:duration 6}))
#+end_src
