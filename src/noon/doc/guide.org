...
The =cat= function takes an arbitrary number of transformations and concatenate their results into one score.

#+begin_src clojure :proll
(play (cat C0 E0 G0 B0))
#+end_src

=cat= accept any valid transformation, here we are using composite transformations.
