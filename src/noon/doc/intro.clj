  ...
  '[:noon
    ...
    [:elements
     ...
     [:top-forms
      ...
      "We can create a score with the =mk= function. With no arguments it simply returns the default score containing only a middle C."
      (mk)
     ...
     [:transformations-1
      ...
      [:composing-transformations
       ...
       {(play [Eb0 dur:2]) "plays a Eb of half duration"
        (play [F#-1 dur4]) "F# above the middle C with quadruple duration."
     ...
     [:transformations-2
      ...
      [:intervals-1
       ...
       [:steps
        ...
        [:chromatic
         ...
         {"going up 3 semitones from wherever we are."
          (c-step 3)
          "going down one semitone."
          (c-step -1)}
        ...
        [:diatonic
         ...
         [:example
          {(play dur:4 (cat d0 d1 d2 d3 d4 d5 d6 d7)) "ascending scale"
           (play dur:4 (cat d0 d2 d1 d3 d2 d4 d3 d5 d4)) "broken scale pattern"
           (play dur:4 (cat d0 d2- d1- d3- d2- d4- d3- d5- d4-)) "same downward"}]
      ...
      [:cat
       ...
       (org :ol
            "We are dividing the duration of our base note by 8."
            "We are creating a series of 3 notes using chromatic intervals (diminished triad C,Eb,Gb)."
            "Then this 3 notes score is passed to each member of the second =cat= expression, each one transposing it from the indicated chromatic interval.")]
     ...
     [:mapping
      ...
      ["One important thing to be aware of is that events will be mapped in place,"
       "so if the given transformation expand the score, some superposition will occur."]
     ...
     [:dynamism
      ...
      ["For now our scores are pretty static, and don't use the power of clojure much."
       "Since this library is built out of simple functions it should be a easy to do so."]
     ...
     [:non-determinism
      ...
      [:!
       ...
       {(play (catn 4 (tup* (shuffle [d0 d2 d4 d6]))))
        "without the bang the shuffle expression is executed only one time."}]
     ...
     [:harmony
      ...
      [:tonality
       ...
       [:struct
        ...
        {noon.constants/structs "full struct list"
         (mk (struct :tetrad)) "sets structure to tetrad"
    ...
    [:composing
     ...
     [:melody
      ...
      [:permutations
       ...
       [:options
        [:grade
         ["The permutations are categorised by grade,"
          "the grade of a permutation correspond to the number of splits that has to be made on the original seq to obtain it."
      ...
      [:contour
       ...
       [:demo
        ...
        {(play (tup s0 s1 s2 s3 s1 s2)
               (m/contour :rotation {:pick -1})) "{:contour [3 0 1 2 0 1]}"}
     ...
     [:rythmn
      ...
      [:transformation
       ...
       [:noon.lib.rythmn/rotation
        ...
        [:example
         ...
         ["You can also rotate relatively to score duration."
          "Here we are starting with a score of duration 2."
          "With the form (r/rotation :relative -1/4) we are rotating it a quarter of its duration backward."]
         (play dur2
               (chans [(patch :tinkle-bell) o1-]
       ...
       [:noon.lib-rythmn/permutation
        ["Like =noon.lib.rythmn/rotation=, =noon.lib.rythmn/permutation= do not operate on a note basis like =noon.lib.melody/permutation=."
        ...
        [:example
         ...
         ["Like we've seen with =noon.lib.melody/permutation=, there is several way to choose a particular permutation."
     ...
     [:harmony
      ...
      [:voicings
       ...
       [:drops
        ...
        [:noon.lib.harmony/drop
         ["This function help you to drop a voicing."
          "It takes the same polymorphic kind of argument (called a 'member-pick') that we've seen with =noon.lib.melody/permutation= and =noon.lib.melody/rotation=."]
      ...
      [:chord-progressions
       ...
       [:melodies
        ...
        [:noon.lib.harmony/align-contexts
