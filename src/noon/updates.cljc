...
(defn* voices
  {:doc "Apply `updates` in parallel on subsequent voices."
   :tags [:base :score-update-builder :parallel]}
  [updates] (fork-with* events/voice+ updates))

(defn* chans
  {:doc "Apply `updates` in parallel on subsequent midi channels."
   :tags [:base :score-update-builder :parallel]}
  [updates] (fork-with* events/chan+ updates))

(defn* tracks
  {:doc "Apply `updates` in parallel on subsequent midi tracks."
   :tags [:base :score-update-builder :parallel]}
  [updates] (fork-with* events/track+ updates))
...
(do :incubator
    ...
    (defn* voices>
      ...
      [updates]
      (par>* (map (fn [i update] (chain (events/voice+ i) update))
                  (range)
                  updates)))
